...
if ($mode == "chklogin") {
    ...
    $result = $query->fetch("SELECT p_id, u_user, u_pass1, u_status FROM tb_hr_user WHERE u_user = ?", $params);
    $hashed_password = password_hash($user_pw, PASSWORD_DEFAULT);

    if (is_array($result) && count($result) > 0) {
        $hashed_password = $result['u_pass1'];
        if (password_verify($user_pw,$hashed_password) ) {
            ...
            $response = array('error' => false, 'message' => 'ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง');
        }
    } else {
        $response = array('error' => false, 'message' => 'ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง');
    }
